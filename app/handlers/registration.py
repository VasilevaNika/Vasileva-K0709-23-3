...
def register_registration_router(router: Router):
    ...
    async def process_photo_upload(
        ...
        # Скачиваем фото из Telegram и загружаем в MinIO (если доступен)
        if storage is not None:
            try:
                photo_bytes_io = await bot.download(file_id)
                storage_key = await storage.upload_photo(photo_bytes_io.read())
                logger.info("Фото загружено в MinIO: %s", storage_key)
            except Exception as exc:
                logger.warning("Не удалось загрузить фото в MinIO: %s", exc)
    ...
    async def process_pref_city(message: Message, state: FSMContext, repo: UserRepository):
        ...
        # Добавляем фото, если есть (сохраняем и Telegram file_id, и MinIO storage_key)
